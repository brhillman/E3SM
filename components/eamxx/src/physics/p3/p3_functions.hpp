...
struct Functions
{
  ...
  KOKKOS_FUNCTION
  static void lookup(const Pack& mu_r, const Pack& lamr,
                     Table3& tab,
  ...
  // ------------------------------------------------------------------------------------------!
  KOKKOS_FUNCTION
  static void lookup_ice(const Pack& qi, const Pack& ni,
  ...
  // tab.dumjj+1}.
  KOKKOS_FUNCTION
  static Pack apply_table(const view_2d_table& table,
                           const Table3& t);
  ...
  static void ice_sedimentation_disp(
    const uview_2d<const Pack>& rho,
    const uview_2d<const Pack>& inv_rho,
    const uview_2d<const Pack>& rhofaci,
    ...
    const uview_2d<Pack>& qi_tend,
    const uview_2d<Pack>& ni_tend,
    const view_ice_table& ice_table_vals,
  ...
  static void rain_water_conservation(
    const Pack& qr, const Pack& qc2qr_autoconv_tend, const Pack& qc2qr_accret_tend, const Pack& qi2qr_melt_tend, const Pack& qc2qr_ice_shed_tend, const Scalar dt,
  ...
  static void calc_rime_density(const Pack& T_atm, const Pack& rhofaci,
    const Pack& table_val_qi_fallspd, const Pack& acn, const Pack& lamc,
    const Pack& mu_c, const Pack& qc_incld, const Pack& qc2qi_collect_tend,
  ...
  KOKKOS_FUNCTION
  static void rain_immersion_freezing(const Pack& T_atm, const Pack& lamr,
    const Pack& mu_r, const Pack& cdistr, const Pack& qr_incld,
    Pack& qr2qi_immers_freeze_tend, Pack& nr2ni_immers_freeze_tend,
    const P3Runtime& runtime_options,
    const Mask& context = Mask(true) );
  ...
  static void ice_classical_nucleation(const Pack& frzimm, const Pack& frzcnt,
    ...
    Pack& nicnt, Pack& qucnt,
    Pack& ninuc_cnt, Pack& qinuc_cnt);

  // Computes droplet self collection
  KOKKOS_FUNCTION
  static void droplet_self_collection(const Pack& rho, const Pack& inv_rho,
    const Pack& qc_incld, const Pack& mu_c, const Pack& nu,
    const Pack& nc2nr_autoconv_tend, Pack& nc_selfcollect_tend,
    const Mask& context = Mask(true) );

  // Computes the accretion of clouds by rain
  KOKKOS_FUNCTION
  static void cloud_rain_accretion(const Pack& rho, const Pack& inv_rho,
    const Pack& qc_incld, const Pack& nc_incld, const Pack& qr_incld, const Pack& inv_qc_relvar,
  ...
  KOKKOS_FUNCTION
  static void rain_self_collection(const Pack& rho, const Pack& qr_incld, const Pack& nr_incld, Pack& nr_selfcollect_tend,
                                   const P3Runtime& runtime_options,
                                   const Mask& context = Mask(true) );

  // Impose maximum ice number
  KOKKOS_FUNCTION
  static void impose_max_total_ni(Pack& ni_local, const Scalar& max_total_ni, const Pack& inv_rho_local,
                                  const Mask& context = Mask(true) );
  ...
  //--------------------------------------------------------------------------------
  KOKKOS_FUNCTION
  static Pack calc_bulk_rho_rime(
  ...
  // TODO (comments)
  KOKKOS_FUNCTION
  static void ice_cldliq_collection(const Pack& rho, const Pack& temp,
  ...
  // TODO (comments)
  KOKKOS_FUNCTION
  static void ice_rain_collection(const Pack& rho, const Pack& temp,
  ...
  KOKKOS_FUNCTION
  static void rain_evap_tscale_weight(const Pack& dt_over_tau,
				      Pack& weight,
				      const Mask& context=Mask(true));

  // helper fn for evaporate_rain
  KOKKOS_FUNCTION
  static void rain_evap_equilib_tend(const Pack& A_c,const Pack& ab,
				     const Pack& tau_eff,const Pack& tau_r,
				     Pack& tend,const Mask& context=Mask(true));

  // helper fn for evaporate_rain
  KOKKOS_FUNCTION
  static void rain_evap_instant_tend(const Pack& ssat_r, const Pack& ab,
				     const Pack& tau_r,
				     Pack& tend, const Mask& context=Mask(true));

  // TODO (comments)
  KOKKOS_FUNCTION
  static void evaporate_rain(const Pack& qr_incld, const Pack& qc_incld, const Pack& nr_incld, const Pack& qi_incld,
			     ...
			     const Pack& epsr, const Pack & epsi_tot, const Pack& t, const Pack& t_prev,
  ...
  KOKKOS_FUNCTION
  static void ice_melting(const Pack& rho, const Pack& T_atm, const Pack& pres, const Pack& rhofaci,
			  ...
			  Pack& qi2qr_melt_tend, Pack& ni2nr_melt_tend, const Mask& context = Mask(true));

  //liquid-phase dependent processes:
  KOKKOS_FUNCTION
  static void update_prognostic_liquid(const Pack& qc2qr_accret_tend, const Pack& nc_accret_tend,
  ...
  KOKKOS_FUNCTION
  static void ice_deposition_sublimation(const Pack& qi_incld, const Pack& ni_incld, const Pack& t_atm, const Pack& qv_sat_l, const Pack& qv_sat_i, const Pack& epsi, const Pack& abi, const Pack& qv, const Scalar& inv_dt, Pack& qidep, Pack& qi2qv_sublim_tend, Pack& ni_sublim_tend, Pack& qiberg, const Mask& context = Mask(true) );
  ...
  // ice nucleation
  KOKKOS_FUNCTION
  static void ice_nucleation(const Pack& temp, const Pack& inv_rho,
                             const Pack& ni, const Pack& ni_activated,
                             const Pack& qv_supersat_i, const Scalar& inv_dt,
  ...
  KOKKOS_FUNCTION
  static void ice_cldliq_wet_growth(const Pack& rho, const Pack& temp, const Pack& pres, const Pack& rhofaci, const Pack& table_val_qi2qr_melting,
                                    ...
                                    Mask& log_wetgrowth, Pack& qr2qi_collect_tend, Pack& qc2qi_collect_tend, Pack& qc_growth_rate,
				    Pack& nr_ice_shed_tend, Pack& qc2qr_ice_shed_tend, const Mask& context = Mask(true));

  KOKKOS_FUNCTION
  static void check_values(const uview_1d<const Pack>& qv, const uview_1d<const Pack>& temp, const Int& ktop, const Int& kbot,
...
#ifdef SCREAM_P3_SMALL_KERNELS
  static void check_values_disp(const uview_2d<const Pack>& qv, const uview_2d<const Pack>& temp, const Int& ktop, const Int& kbot,
  ...
  static void p3_main_init_disp(
    ...
    const uview_2d<const Pack>& cld_frac_r, const uview_2d<const Pack>& inv_exner,
    const uview_2d<const Pack>& th_atm, const uview_2d<const Pack>& dz,
    const uview_2d<Pack>& diag_equiv_reflectivity, const uview_2d<Pack>& ze_ice,
    const uview_2d<Pack>& ze_rain, const uview_2d<Pack>& diag_eff_radius_qc,
    const uview_2d<Pack>& diag_eff_radius_qi, const uview_2d<Pack>& diag_eff_radius_qr, const uview_2d<Pack>& inv_cld_frac_i,
    ...
    const uview_2d<Pack>& inv_dz, const uview_1d<Scalar>& precip_liq_surf, const uview_1d<Scalar>& precip_ice_surf,
  ...
  static void p3_main_part3(
    ...
    const uview_1d<Pack>& ze_rain,
    const uview_1d<Pack>& ze_ice,
    const uview_1d<Pack>& diag_vm_qi,
    const uview_1d<Pack>& diag_eff_radius_qi,
    const uview_1d<Pack>& diag_diam_qi,
    const uview_1d<Pack>& rho_qi,
    const uview_1d<Pack>& diag_equiv_reflectivity,
    const uview_1d<Pack>& diag_eff_radius_qc,
    const uview_1d<Pack>& diag_eff_radius_qr,
    const P3Runtime& runtime_options);
  ...
  KOKKOS_FUNCTION
  static void ice_supersat_conservation(Pack& qidep, Pack& qinuc, Pack& qinuc_cnt, const Pack& cld_frac_i, const Pack& qv, const Pack& qv_sat_i, const Pack& t_atm, const Real& dt, const Pack& qi2qv_sublim_tend, const Pack& qr2qv_evap_tend, const bool& use_hetfrz_classnuc, const Mask& context = Mask(true));

  KOKKOS_FUNCTION
  static void nc_conservation(const Pack& nc, const Pack& nc_selfcollect_tend, const Real& dt, Pack& nc_collect_tend, Pack& nc2ni_immers_freeze_tend, Pack& nc_accret_tend, Pack& nc2nr_autoconv_tend, Pack& ncheti_cnt, Pack& nicnt, const bool& use_hetfrz_classnuc, const Mask& context = Mask(true));

  KOKKOS_FUNCTION
  static void nr_conservation(const Pack& nr, const Pack& ni2nr_melt_tend, const Pack& nr_ice_shed_tend, const Pack& ncshdc, const Pack& nc2nr_autoconv_tend, const Real& dt, const Real& nmltratio, Pack& nr_collect_tend, Pack& nr2ni_immers_freeze_tend, Pack& nr_selfcollect_tend, Pack& nr_evap_tend, const Mask& context = Mask(true));

  KOKKOS_FUNCTION
  static void ni_conservation(const Pack& ni, const Pack& ni_nucleat_tend, const Pack& nr2ni_immers_freeze_tend, const Pack& nc2ni_immers_freeze_tend, const Pack& ncheti_cnt, const Pack& nicnt, const Pack& ninuc_cnt, const Real& dt, Pack& ni2nr_melt_tend, Pack& ni_sublim_tend, Pack& ni_selfcollect_tend, const bool& use_hetfrz_classnuc, const Mask& context = Mask(true));

  KOKKOS_FUNCTION
  static void prevent_liq_supersaturation(const Pack& pres, const Pack& t_atm, const Pack& qv, const Scalar& dt, const Pack& qidep, const Pack& qinuc, Pack& qi2qv_sublim_tend, Pack& qr2qv_evap_tend, const Mask& context = Mask(true) );
}; // struct Functions
